...
### 13. En su metodología (Pág. 27 del PDF) indica que empleará la prueba t de Student para muestras relacionadas. ¿Cómo la aplica en su proyecto y por qué eligió "muestras relacionadas"?
* **Respuesta del Tesista**:
  > *"Elegimos la prueba **t de Student para muestras relacionadas** (en Python, `scipy.stats.ttest_rel`) porque evaluamos a las mismas unidades de análisis (el tiempo de diagnóstico) en dos momentos correlacionados sobre los mismos sujetos: antes del chatbot (Pre-test) y después del chatbot (Post-test). El análisis matemático calcula la diferencia de las medias y evalúa su significancia. Dado que nuestro P-Valor resultante es menor a 0.05, rechazamos la hipótesis nula y validamos científicamente que el chatbot optimiza significativamente la eficiencia del taller."*
